...
///
/// iOS, iPadOS and macOS: tvOS's `UNMutableNotificationContent` exposes no title/body/sound, so
/// there is no useful local "ready" alert on the TV — these methods are no-ops there (the title
/// still flips into the library via the library refresh).
@MainActor
