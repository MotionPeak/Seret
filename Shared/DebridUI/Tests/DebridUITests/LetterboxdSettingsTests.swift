...
@Suite struct LetterboxdSettingsTests {
    ...
    @Test func aServerAddressBecomesAUsableURL() {
        #expect(LetterboxdSettings(serverURL: "192.168.1.179:8080").serverBaseURL?.absoluteString
                == "http://192.168.1.179:8080")
        #expect(LetterboxdSettings(serverURL: "http://nas.local:8080").serverBaseURL?.absoluteString
                == "http://nas.local:8080")
        #expect(LetterboxdSettings(serverURL: "  192.168.1.179:8080  ").serverBaseURL?.absoluteString
                == "http://192.168.1.179:8080")
    }
    ...
    @Test func anEmptyAddressIsNoServer() {
        #expect(LetterboxdSettings(serverURL: "").serverBaseURL == nil)
        #expect(LetterboxdSettings(serverURL: "   ").serverBaseURL == nil)
    }
    ...
    @Test func theAddressSurvivesTheRoundTripThroughTheStore() {
        ...
        store.save(LetterboxdSettings(username: "thebigshin", serverURL: "nas.local:8080"))
        #expect(store.load().serverURL == "nas.local:8080")
    ...
    @Test func settingsWrittenBeforeTheFieldExistedStillLoad() throws {
        ...
        #expect(decoded.serverURL.isEmpty)
    }
}
