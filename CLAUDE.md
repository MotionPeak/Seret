...
### Plan 8 (iPhone/iPad universal app) — DONE on `feat/mobile-foundation` (pushed; NOT merged to `main`; real-playback owner-pending)

Built 2026-06-06 (brainstorm w/ visual companion → spec [`docs/superpowers/specs/2026-06-06-mobile-foundation-signin-design.md`] → inline execution; owner co-edited the tvOS player in parallel). The iPhone/iPad app is **feature-complete for Stage 1 (browse · organize · play)** on the shared brain.
- **8a — shared layer + foundation:** extracted **`Shared/DebridUI`** (the presentation layer: all view-models [`AppSession`/`SignInModel`/`LibraryStore`/`DetailStore`/`SettingsModel`/`PlayerModel`] + provider seams + models + utilities + design `Tokens`) out of `SeretTV` so both apps share it — the 5 view-model suites now run **host-free under `swift test`** (48 tests). New universal **`SeretMobile`** target (iOS 18, `TARGETED_DEVICE_FAMILY "1,2"`, bundle `com.solomons.seret.mobile`): adaptive `MainShell` (size-class → `TabView` iPhone / `NavigationSplitView` iPad), touch `SignInView` (device-code + paste-token via `SFSafariViewController`), iOS Settings `Form`.
- **8b — library + Detail:** adaptive poster grid (~3-col iPhone / ~5-col iPad) on `LibraryStore`; movie/show Detail + episodes on `DetailStore`. Native-per-platform iOS views.
- **8c — player:** shared **`Shared/SeretPlayer`** (`VLCKitVideoPlayerEngine` + `VLCVideoView`, compiled into BOTH apps via `project.yml` `sources:`; the unified `VLCKit.xcframework` has iOS slices, embedded in SeretMobile) + touch **`PlayerView`** (Balanced gestures: tap toggles controls, double-tap each half ±10s, drag-scrub; transport + loading/error overlays) + **`PlayerSettingsSheet`** (audio/subtitle tracks + on-demand he/en subs + 0.5–1.5x speed); `PlayerModel.toggleControls()` added; the owner had simplified `AppSession.makePlayer` to `(for:engine:)` (dropped the live-frame scrub preview).
- **Verified:** SeretMobile builds 0/0 + launches with VLCKit (no dyld crash); SeretTV 0/0 (no regression); DebridCore 130 + DebridUI 48 `swift test` green. ⚠️ **Real RD-stream playback OWNER-PENDING** — the player is only reachable after sign-in (no token → the library shows the failed-state), so actual video needs the owner's RD token on the sim / a device (same as the tvOS player DoD).
- **Gotcha:** the iOS **26.5 simulator runtime** must be installed (the SDK is 26.5; if only 26.4 is present, CLI `xcodebuild` reports "found no destinations").
- **▶ NEXT:** owner verifies real playback (sign in with the RD token) → **merge `feat/mobile-foundation` → `main`** → Home (Continue Watching / Recently Added) + Stage 2 (Search → Add).
...
DebridCore (pure Swift, no UI, no VLCKit, unit-tested)
  ...
  ✓ Subtitles(SubtitleProvider + OpenSubtitles)   ✓ Persistence(SwiftData cache + WatchProgress)   ✓ Playback(VideoPlayerEngine + PlaybackCoordinator)
        ▲                                   ▲
   SeretTV (tvOS)                     SeretMobile (iOS/iPadOS)   ✓ both built (Plans 7–8; iOS on feat/mobile-foundation)
