...
## Status — **Plans 1–6 + 7a + 7b DONE** — `DebridCore` brain feature-complete **and the `SeretTV` tvOS app (sign-in + library grids + Detail/episodes drill-down) merged to `main`**; Plan 7c (VLCKit player) next
...
### ▶ RESUME HERE — Plan 7 sliced 7a/7b/7c; **7a + 7b (i + ii) DONE & merged to `main`**
...
**▶ NEXT = Plan 7c (player):** the real **VLCKit player** — a `VideoPlayerEngine` impl + a `PlayerView` replacing `PlayerPlaceholderView` (consumes the `PlaybackRequest` seam 7b-ii built: it carries the chosen `MediaSource` + `resumeAt`), **unrestrict at play time** (`TorrentsClient.playableURL`), on-demand subtitles (`OpenSubtitlesProvider`, needs the key), and resume-write via `PlaybackCoordinator`. `TVVLCKit.xcframework` integrated here (fetch `Scripts/fetch-frameworks.sh`, embed via XcodeGen like Nikud's `llama.xcframework`). Say **"start Plan 7c"** → brainstorm → spec → plan → execute. **Later:** Home (hero + Continue Watching [`WatchProgressStore.recentlyWatched()` exists] + Recently Added [needs `addedAt` on `MediaItem`]); Search/Add (Stage 2); **Plan 8** = iPhone/iPad (`MobileVLCKit`).
...
- **This dev environment can't launch the tvOS Simulator** — `xcodebuild test` and SwiftUI previews fail at runner-launch with `Pseudo Terminal Setup Error, ErrorCode 7, Errno 6 ("Device not configured")` on `appletvsimulator26.5` (persists even with the sandbox off — it's the headless context, not a code bug). What DOES work in an agent session: `swift test` (DebridCore, no sim) and `xcodebuild build`/`build-for-testing` (compile, no launch). So **app-target unit tests + sim screenshots must be run from the Xcode GUI (⌘U / canvas)** by the owner. (Distinct from the Claude.app pty-pool exhaustion error.)
