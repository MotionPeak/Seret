...
## Status — **Plans 1–6 + 7a + 7b-i DONE** — `DebridCore` brain feature-complete **and the `SeretTV` tvOS app (device-code sign-in + library grids) merged to `main`**; Plan 7b-ii (Detail + episodes) next

**The `DebridCore` package is real, on `main` ([github.com/MotionPeak/Seret](https://github.com/MotionPeak/Seret)) — 112 tests green, zero warnings.** Built test-first via the superpowers pipeline (brainstorm → spec → one plan per slice → subagent-driven TDD), each plan adversarially reviewed (spec + code-quality + final) before a fast-forward merge to `main`.
...
### ▶ RESUME HERE — Plan 7 sliced 7a/7b/7c; **7a + 7b-i DONE & merged to `main`**
...
**Deferred formality (both 7a + 7b-i):** the app's own DoD **screenshots** (sign-in code → grids with real posters) need **one cold RD device-code sign-in** — the sim isn't signed in and RD rate-limits `device/code` HARD (see Gotchas). The code is verified by build + tests + a live host-side auth round-trip; only the live screenshots await the throttle cooling.

**▶ NEXT = Plan 7b-ii (library drill-down):** the **Detail** screen (backdrop-forward, Resume/Play, synopsis, quality/source chips derived from `MediaSource.parsed`) + **Show/episodes** (season picker, episode rows, watch-progress bars). Wire poster selection in `PosterCard` (a no-op in 7b-i) → Detail. Say **"start Plan 7b-ii"** → brainstorm → spec → plan → execute subagent-driven (verify each screen in the tvOS sim before claiming done). **Then 7c** = player (VLCKit `VideoPlayerEngine` impl + on-demand subs + resume; needs the OpenSubtitles key; `TVVLCKit` integrated here). **Later:** Home (hero + Continue Watching + Recently Added — needs 7c watch-history + an `addedAt` on `MediaItem`); Search/Add (Stage 2); **Plan 8** = iPhone/iPad from the same brain (tab bar / `NavigationSplitView` + `MobileVLCKit`).
