...
@Suite struct VersionGroupTests {
    ...
    @Test func everyInstantVersionComesBeforeEveryDownload() throws {
        ...
        try #require(groups.map(\.availability) == [.instant, .download])
        #expect(groups[0].rest.map(\.infoHash) == ["i1", "i2"])
        #expect(groups[1].rest.map(\.infoHash) == ["d1", "d2"])
    }

    @Test func theRankingHoldsInsideEachGroup() throws {
        ...
        let instant = try #require(list.groupedByAvailability(episodesInSeason: nil).first)
        #expect(instant.rest.map(\.infoHash) == ["i-best", "i-next", "i-last"])
    }

    @Test func eachGroupSplitsOffItsOwnOversizedReleases() throws {
    ...
    @Test func aSeasonPackIsJudgedPerEpisodeInsideItsGroup() throws {
        ...
        let byTen = try #require(pack.groupedByAvailability(episodesInSeason: 10).first)
        let byThirty = try #require(pack.groupedByAvailability(episodesInSeason: 30).first)
        #expect(byTen.larger.map(\.infoHash) == ["pack"])
        #expect(byThirty.larger.isEmpty)
    }
}
